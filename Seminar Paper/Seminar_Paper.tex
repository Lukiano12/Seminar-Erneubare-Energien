...
\IEEEauthorblockA{\textit{Electrical Energy Systems and Electromobility (M.Eng.)} \\
...
% --- BILD 3: Z-Scheme Placeholder ---
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{images/zscheme.png}    \vspace{0.2cm}
    \caption{The biological Z-scheme illustrating the two upward energy boosts required in natural photosynthesis \cite{nelsonStructureFunctionPhotosystems2006} \cite{qiReviewTiO2basedZscheme2017}.}
    \label{fig:z_scheme}
\end{figure*}
% ------------------------------------
...
This process relies heavily on an enzyme called RuBisCO. While important, RuBisCO is famously slow and inefficient: it has a turnover rate of only about 3 to 10 reactions per second, compared to thousands of reactions per second for typical enzymes \cite{anderssonCatalysisRegulationRubisco2008}. Furthermore, it sometimes grabs oxygen by mistake instead of $CO_2$, which wastes energy. It is a slow assembly line, trading speed for the ability to build incredibly dense and stable fuel at normal outdoor temperatures.
...
\begin{figure*}[htbp]
    ...
    \vspace{0.2cm}
    \caption{Energy loss cascade in natural photosynthesis. Despite high efficiency in early light absorption, overall system losses reduce the final biological storage efficiency drastically, leading to real-world limits of 1\% to 2\% \cite{zhuImprovingPhotosyntheticEfficiency2010}. Image generated using Perplexity AI.}
